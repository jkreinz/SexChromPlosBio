...
plink <- read.table("data_by_script/03_PCA/herb_commongarden_pnas_missing9_maf05.fam")
names(plink)[1] <- "sample"
plink$order <- seq(1, nrow(plink))

# Read comprehensive sample information
allmeta <- read.table("data_by_script/03_PCA/3waymerged_sampleinfo.txt", sep = "\t", header = TRUE)

# Join with PLINK order
both2 <- inner_join(plink, allmeta, by = "sample")

# Read metadata with ancestry information
meta_ordered <- read.table("data_by_script/03_PCA/commongarden_and_PNAS_metadata_ancestry.txt", 
                           header = TRUE)
...
e_val<-read.table("data_by_script/03_PCA/commongarden_allfiltsnps_193_hap2_remvdrepeats_outliersdropped_scaf1inversion.eigenval")
contemp_herb<-fread("data_by_script/03_PCA/commongarden_allfiltsnps_193_hap2_remvdrepeats_outliersdropped_scaf1inversion.eigenvec")
...
for (inversion_num in 1:18) {
  ...
  eigenvec_file <- paste0("data_by_script/03_PCA/inversion", inversion_num, ".eigenvec")
  eigenval_file <- paste0("data_by_script/03_PCA/inversion", inversion_num, ".eigenval")
