# ===========================================
# GWAS and Pop Gen Analyses of sex-linkage 
# ===========================================
...
#read in data
gwa<-fread("depth_by_script/01_GWAS/commongarden_allfiltsnps_193_hap2_remvdrepeats_maf05.assoc.txt")
...
p1a <- gwa %>% 
  ...
  facet_wrap(chr_numeric, scales="free_x",nrow = 1)
...
####SUP FIGURE 2
#read in data
gwa<-fread("depth_by_script/01_GWAS/sexassocsnps_nomultimapping_recalled_metareorder.assoc.txt")
...
###SUP FIGURE 3
#read in data
gwa<-fread("depth_by_script/01_GWAS/193_XYScaffolds_filteredsnps.assoc.txt")
...
tophit<- read.table("depth_by_script/01_GWAS/tophit.012",header = F)
th_ind<-read.table("depth_by_script/01_GWAS/tophit.012.indv",header = F)
tophit$ind<-th_ind$V1
th_meta<- fread("depth_by_script/01_GWAS/commongarden_allfiltsnps_193_hap2.fam")
th_meta$ind<-paste(th_meta$V1,th_meta$V2,sep="_")
sex<-read.table("depth_by_script/01_GWAS/sexpheno")
...
genos<-read.table("depth_by_script/01_GWAS/sexassocsnps_nomultimapping_recalled.raw",header=T) #012 data of sex linked snps
snppos<-read.table("depth_by_script/01_GWAS/sexassocsnps_nomultimapping_recalled.vcf.bim")
...
fst_all<-fread("depth_by_script/01_GWAS/male_female_1kb_fst.txt")
head(fst_all)
...
malefemale_hetprop<-fread("depth_by_script/01_GWAS/male_female_hetprop.txt")
...
genes<-read.table("depth_by_script/01_GWAS/193_2_genes_100kbdensity.bed")
#TEs<-fread("allrepeats_densities_10kb.bed")
TEs<-read.table("depth_by_script/01_GWAS/193_2_TEs_100kbdensity.bed")

names(genes)<-c("chr","start","end","count","density")
names(TEs)<-c("chr","start","end","count","density")
div<-read.table("depth_by_script/01_GWAS/0fold_allinds_allsites_100kb_pi.txt",header = T)
