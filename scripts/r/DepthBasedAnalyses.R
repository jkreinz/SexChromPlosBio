...
all <- read.table("data_by_script/04_Depth/herbcontemp_poplist.txt", 
                  na.strings = c("", "NA", "?"), sep = "\t", header = F)
names(all)[1] <- "sample"

plink <- read.table("data_by_script/04_Depth/herb_commongarden_pnas_missing9_maf05.fam")
...
all2 <- read.table("data_by_script/04_Depth/3waymerged_sampleinfo.txt", sep = "\t", header = T)
both2 <- inner_join(plink, all2, by = "sample")

# Read PCA results from common garden analysis
contemp_herb <- fread("data_by_script/04_Depth/commongarden_allfiltsnps_193_hap2.eigenvec")
...
meta_ordered <- read.table(
  "data_by_script/04_Depth/commongarden_and_PNAS_metadata_ancestry.txt",
  header = T
)
...
# Read mismatched samples
mispheno <- read.table("data_by_script/04_Depth/misphenoed_samples.txt")
...
depthmat <- fread("data_by_script/04_Depth/allsamples_numericorder_depthsby100bpwind_tab.txt")
...
cg <- read.table("data_by_script/04_Depth/3waymerged_sampleinfo.txt", 
...
phenos<-fread("data_by_script/04_Depth/Common Garden Seq Stats - geno w pheno.csv")
...
depthmat<-fread("data_by_script/04_Depth/allsamples_numericorder_sexhitsdepth_tab.txt",sep="\t",sep2 = " ")
depthmat[1:10,1:10]
names(depthmat)<-c("scaf","start","end",str_sort(contemp_herb_pca_anc$sample, numeric = F))
