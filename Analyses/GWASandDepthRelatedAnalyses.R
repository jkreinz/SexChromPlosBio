# ==============================================================================
# GWAS and Individual Depth Analyses - Figure 1A and B, Figure 3C, Sup Fig XXX, 
# ==============================================================================
...
fst_all %>% #filter(CHR == "Scaffold_1Y") %>%  filter(POS > 38000000 & POS < 47000000) %>%
  #mutate(chr_numeric = as.numeric(str_extract(chr, "\\d+"))) %>%
  #filter(chr_numeric >= 1 & chr_numeric <= 16) %>%
  ...
  # geom_hline(yintercept = -log10(max(gwa_FDR$p_wald)),lty="dashed",lwd=.5) +
  #geom_hline(yintercept = -log10(max(gwa_bon$p_wald)),lty="dashed",lwd=.25,color="black") +
...
quantile(fst_all[fst_all$chromosome != "Scaffold_1",]$avg_wc_fst, probs=c(0.025,0.975))

p3<-fst_all %>% filter(chromosome == "Scaffold_1") %>%  filter(window_pos_1 > 30000000 & window_pos_1 < 48000000) %>%
  ggplot(aes(window_pos_1/1000000,avg_wc_fst)) +
  geom_point(alpha=.2) +
